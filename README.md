# <img src="https://github.com/xiaoBingge114514/VRChat-OSC-Chat-Tool/blob/main/VRChat-OSC-Say!.ico" width="35" height="35"> VRChat-OSC-Chat-Tool v1.4

为VRChat的聊天消息制作的OSC ChatBox 工具程序

* 欢迎你为程序提出新功能的建议！
* 喜欢本项目记得点个⭐Star噢~

---

## ✨ 最新更新（v1.4 - 2026.6.18）

### 🔧 重构了代码结构
- 单文件结构已被舍弃，现在将采用多文件结构格式
- 感谢[@MineAurora](https://github.com/MineAurora2)的技术支持

### 🎉 新增功能
- **打开软件自动发送** - 在 设置 → 发送设置 中添加了一个功能选项，允许用户在打开软件经过一段时间自动开始发送消息
- **支持AMD显卡性能监测** - AMD的GPU现在可以正常使用性能监测了
---

## 📸 预览
<img width="1503" height="911" alt="QQ20260408-043418" src="https://github.com/user-attachments/assets/dfd6a367-bfa7-40bf-bf5c-5682f13ed326" />

---

## 📞 有 需求/问题 吗？

* 在 Discord 上添加我的好友并给我发消息！
  * 我的名字： bili_xiaobingge
* 在 BiliBili 上通过私信给我发消息！
  * https://space.bilibili.com/65102520 
* 发个Issues ？！
  
---

## 🚀 软件特征

### 核心功能
* ✅ **文本消息** - 支持最多144字符的消息发送，实时显示剩余可用字符数
* ✅ **历史记录** - 保存最近20条发送记录
* ✅ **自动发送** - 可设置5-300秒的间隔自动发送
* ✅ **OSC端口配置** - 支持自定义OSC通信端口

### 智能附加项
* ✅ **当前电脑时间** - 自动附加 UTC+8 格式时间
* ✅ **当前窗口标题** - 获取当前焦点窗口名称（支持字符数限制）
* ✅ **音乐信息** - 使用 SMTC 协议获取歌曲信息（支持网易云音乐等，可限制标题/艺术家字符数）
* ✅ **挂机检测** - 检测离开电脑的时长，5-21600秒阈值可调，超过阈值自动显示
* ✅ **硬件监测** - CPU、RAM、GPU 占用信息，支持自定义标签（支持 NVIDIA 和 AMD 显卡（**NEW!**））
* ✅ **心率监测** - 蓝牙直连手环/手表获取实时心率

### 个性化设置
* ✅ **发送顺序调整** - 自由排列消息内容与各项附加信息的先后顺序
* ✅ **硬件标签自定义** - 自定义硬件监测项的显示名称
* ✅ **字符数限制** -控制窗口标题和音乐信息的显示长度
* ✅ **发送设置** - 控制OSC发送的地址以及端口
* ✅ **模板字符串模式** - 高级消息格式化功能，支持多种变量
* ✅ **高级音乐信息** - 网易云音乐歌词同步
* ✅ **启动自动发送消息** - 启动软件自动发送消息（**NEW!**）

---

## 📦 无需安装

* 只需下载文件运行即可

---

## 🛠️ 本地构建

### 项目结构

```text
VRChat-OSC-Chat-Tool/
├─ main.py                  # 程序入口文件，用于启动整个应用
├─ config.py                # 配置管理模块，负责读取、保存和处理程序配置
├─ osc_sender.py            # OSC 消息发送模块，用于向 VRChat 发送聊天或控制消息
├─ hardware_monitor.py      # 硬件状态监控模块，用于获取 CPU、内存等系统信息
├─ ble_heartrate.py         # 蓝牙心率模块，用于读取 BLE 心率设备数据
├─ netease_sync.py          # 网易云音乐同步模块，用于获取当前播放歌曲信息
├─ vrchat_config.json       # VRChat OSC 配置文件，保存运行所需的相关参数（软件运行后生成）
├─ VRChat-OSC-Say!.ico      # 应用图标文件
└─ gui/
   ├─ __init__.py           # GUI 包初始化文件
   ├─ app.py                # 图形界面主程序，负责创建和运行主窗口
   ├─ widgets.py            # 自定义界面组件模块，封装可复用的 GUI 控件
   ├─ settings_panel.py     # 设置面板模块，用于管理用户可调整的选项
   ├─ message_logic.py      # 消息逻辑模块，负责处理消息生成、更新和发送逻辑
   └─ config_panel.py       # 配置面板模块，用于展示和编辑程序配置
```

### 环境要求
* Python 3.11+ https://www.python.org/downloads/ 

### 安装依赖

```bash
pip install python-osc
pip install pywin32
pip install psutil
pip install nvidia-ml-py
pip install pyadl
pip install winsdk
pip install bleak
pip install websockets
pip install pydantic
pip install PyInstaller
pip install auto-py-to-exe
```

---

### 📚 依赖库说明

 库名 | 作用 |
 |:--------:|:--------:|
|tkinter | GUI界面构建 |
|python-osc |	OSC协议通信 |
|pywin32 | Windows窗口信息获取 |
|psutil | 系统资源监控 |
|nvidia-ml-py | GPU信息获取（NVIDIA） |
|pyadl | GPU信息获取（AMD） |
|winsdk | Windows媒体控制接口(SMTC) |
|bleak | 蓝牙(BLE)通信，用于心率监测 |
|websockets | 新增 - WebSocket通信，用于网易云音乐同步 |
|pydantic | 新增 - 数据验证和设置管理 |

---

### ❤️ 心率监测使用说明

* 支持的设备
* 小米手环 8/9/10
* 小米手表
* 其他支持 HRS (Heart Rate Service) 协议的蓝牙心率设备

### 使用方法
1. 确保你的设备已开启心率广播功能
2. 在程序中勾选"心率检测"选项
3. 程序会自动扫描并连接附近的蓝牙心率设备
4. 连接成功后，心率数据会显示在调试面板中
5. 发送消息时会自动附加 [❤️:XXX BPM] 格式的心率信息

---

### ⚙️ 设置功能说明

点击主界面"设置"按钮可打开设置窗口，包含四个标签页：

   ### 发送顺序
  * 通过上移/下移按钮调整各项内容的排列顺序
  * 支持排序的项包括：消息内容、时间、挂机状态、窗口标题、音乐信息、心率、硬件监测
  * 在模板模式下，此功能会被禁用
  ### 硬件标签
  * 自定义CPU、RAM、GPU的显示标签
  * 留空则使用默认格式（如"CPU: 45%"）
  * 设置标签后格式变为"CPU(标签): 45%"
  ### 字符限制
  * 窗口标题最大字符数：1-100（默认20）
  * 音乐标题最大字符数：1-100（默认30）
  * 音乐艺术家最大字符数：1-100（默认30）

  ### 进阶音乐信息
  * 启用高级音乐信息（替换普通音乐信息）
  * 提供启动网易云、选择网易云路径、开始同步歌词等功能
  * 可自定义进度条宽度、填充字符、滑块字符、空白字符
  * 支持自定义音乐信息模板，使用{song}, {artist}, {bar}, {time}, {lyric1}, {lyric2}变量

 ### 发送设置
 * 可以设置消息发送到OSC的什么IP地址以及端口
 * 可以设置启动软件后在多久后开始发送信息（0=立即发送）（1.4新增）

  ### (高级)发送顺序
  * 启用模板字符串模式，使用高级格式化功能
  * 提供快速插入附加项的按钮，包括消息内容、时间、窗口标题、挂机状态、音乐信息、硬件监测、心率、换行符等
  * 可自定义模板字符串，支持UTF-8字符

---

## 📝 字符串模板
* 适用于纯音乐+两个字的简易字符串模板！
<pre>
{time}❀{message}❀{window}\n{hardware}\n{heart_rate}   {idle}\n{music}
</pre>
<img width="427" height="321" alt="image" src="https://github.com/user-attachments/assets/de3bdae2-c4e5-4ae1-85f3-9fc11a2e03c9" />


---

### 🔮 未来计划

* [ ] 我想到的好功能都会写进去，你也可以为我提出 issues

---

### 📝 更新日志
   #### v1.4 (2026.06.18)
   *  🔧 更改了文件结构格式，现在为多文件结构格式（不影响正式发布为单文件的软件格式）
   * ✨ 新增了打开软件自动发送消息功能
   * ✨ 添加了性能检测对AMD显卡的支持

   #### v1.3 (2026.04.08)
   * ✨ 新增模板字符串模式（高级功能）
   * ✨ 新增高级音乐信息功能（网易云歌词同步）
   * ✨ 新增高级音乐信息设置界面
   * ✨ 优化UI界面布局和显示效果
   * 🔧 修复部分UI界面显示问题
   * 🔧 默认发送间隔调整为3秒（原10秒）
   * 🔧 为程序添加了一个图标

   #### v1.2 (2026.03.05)
   * ✨ 新增OSC端口自定义功能
   * ✨ 新增发送顺序自定义功能
   * ✨ 新增硬件标签自定义功能
   * ✨ 新增字符数限制设置（窗口标题、音乐标题/艺术家）
   * ✨ 配置保存功能
   * 🔧 挂机检测阈值下限调整为5秒（原60秒）
   * 🔧 默认发送间隔调整为10秒（原15秒）
   * 🔧 优化硬件监测显示格式

   #### v1.1 (2026.02.15)
   * ✨ 新增蓝牙心率监测功能（基于 HRS 协议）
  
   #### v1.0 (2025.03.04)
   * 🎉 初始版本发布
   * 基础OSC消息发送
   * 时间/窗口/音乐/挂机检测
   * 硬件监测功能
